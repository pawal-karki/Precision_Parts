...
function StatCard({ label, value, delay = 0, className = "" }) {
  return (
    <motion.div
      className={`bg-surface-container-low dark:bg-stone-900 rounded-xl p-6 flex flex-col justify-between ${className}`}
      initial={{ opacity: 0, y: 16 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ delay }}
    >
      <p className="text-xs font-bold uppercase tracking-widest text-on-surface-variant mb-1">{label}</p>
      <h2 className="text-2xl sm:text-3xl font-extrabold font-headline text-on-surface dark:text-white">{value}</h2>
    </motion.div>
  );
}
    ...
    <PageTransition>
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <motion.div
            className="bg-tertiary-container rounded-xl p-6 flex flex-col justify-between"
            initial={{ opacity: 0, y: 16 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            ...
            <h2 className="text-2xl font-extrabold font-headline text-on-tertiary-container">
              {pendingPayments}
            </h2>
            <Button
              variant="surface"
              className="mt-3 w-full bg-white/60 hover:bg-white text-on-tertiary-container border border-white/20 text-xs"
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          {/* AI Health Alerts */}
          <motion.div
            className="md:col-span-12 lg:col-span-6 bg-surface-container-lowest dark:bg-stone-900 rounded-xl p-6 shadow-sm ring-1 ring-surface-container"
            initial={{ opacity: 0, y: 16 }}
            animate={{ opacity: 1, y: 0 }}
            ...
            <div className="space-y-4">
              {vehicles.length > 0 ? (
                vehicles.slice(0, 2).map((v, i) => (
                  <div key={v.id || i} className={`flex items-center gap-4 p-3 rounded-lg ${i === 0 ? "bg-tertiary-fixed" : "bg-secondary-fixed"}`}>
                    <div className={`w-10 h-10 flex items-center justify-center rounded-full bg-white ${i === 0 ? "text-tertiary" : "text-secondary"}`}>
                      <Icon name={i === 0 ? "slow_motion_video" : "tire_repair"} />
                    </div>
                    <div className="flex-1">
                      <h4 className={`text-sm font-bold ${i === 0 ? "text-on-tertiary-fixed" : "text-on-secondary-fixed"}`}>
                        {i === 0 ? "Brake Pad Wear" : "Tire Pressure Optimization"}
                      </h4>
                      <p className={`text-xs ${i === 0 ? "text-on-tertiary-fixed-variant" : "text-on-secondary-fixed-variant"}`}>
                        {v.nickname || v.name || "Vehicle"}: {v.healthScore >= 90 ? "All systems nominal" : "maintenance recommended"}
                      </p>
                    </div>
                    <span className={`text-xs font-bold ${v.healthScore < 70 ? "text-error" : "text-secondary"}`}>
                ...
                <div className="text-center py-8 text-on-surface-variant">
                  ...
                  <Button variant="outline" className="mt-4" onClick={() => navigate("/customer/profile")}>
          ...
          <div className="md:col-span-12 lg:col-span-6 space-y-4">
            <div className="flex items-baseline justify-between">
              <h3 className="font-headline font-bold text-xl">Managed Vehicles</h3>
              <Link
                to="/customer/profile"
                  ...
                  >
                    ...
                    <div className="p-4">
                      <div className="flex justify-between items-start mb-3">
                        <h4 className="font-headline font-extrabold text-base">{vehicle.nickname || vehicle.name || "My Vehicle"}</h4>
                        <span className={`px-2 py-1 text-[10px] font-bold rounded-full ${(vehicle.healthScore || 100) >= 90 ? "bg-on-secondary-container text-white" : "bg-tertiary-container text-on-tertiary-container"}`}>
                          {(vehicle.healthScore || 100) >= 90 ? "OPTIMAL" : "SERVICE DUE"}
                        </span>
                      </div>
                      <div className="grid grid-cols-2 gap-3">
                        <div className="bg-white/50 dark:bg-stone-800 p-2 rounded-lg">
                          <p className="text-[10px] uppercase font-bold text-on-surface-variant mb-1">Mileage</p>
                          <p className="text-sm font-bold">{vehicle.mileageKm ?? "—"} km</p>
                        </div>
                        <div className="bg-white/50 dark:bg-stone-800 p-2 rounded-lg">
                          <p className="text-[10px] uppercase font-bold text-on-surface-variant mb-1">Health</p>
                          <p className={`text-sm font-bold ${(vehicle.healthScore || 100) >= 90 ? "text-emerald-600" : (vehicle.healthScore || 100) >= 70 ? "text-amber-600" : "text-error"}`}>
                            {vehicle.healthScore || 100} / 100
                          </p>
                        </div>
                      </div>
                    </div>
          ...
          >
            <div className="bg-surface-container-lowest dark:bg-stone-900 rounded-xl p-6 shadow-sm border border-surface-container">
              <h3 className="font-headline font-bold text-xl mb-6">Recent Activity</h3>
              {activity.length === 0 ? (
                <p className="text-center text-on-surface-variant py-6 text-sm">No recent activity yet.</p>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {activity.slice(0, 6).map((item, index) => (
                    <div key={item.id || index} className="flex gap-4">
                      <div className="flex flex-col items-center">
                        <div className={`w-2 h-2 rounded-full mt-1.5 ${index === 0 ? "bg-secondary" : "bg-surface-container-highest"}`} />
                      </div>
                      <div className="flex-1 pb-4 border-b border-surface-container dark:border-stone-800 last:border-0">
                        <p className="text-xs font-bold text-on-surface-variant uppercase mb-1">{item.date}</p>
              ...
              <Link
                to="/customer/orders"
                className="mt-4 block text-center text-xs font-bold text-secondary uppercase tracking-widest py-3 border-t border-surface-container hover:bg-surface-container-low transition-all rounded-b-xl"
              >
                View Full History
              </Link>
            </div>
        ...
        <div className="rounded-xl border border-secondary/20 bg-secondary/5 p-4">
          <p className="text-sm text-on-surface-variant">
            <span className="font-semibold text-secondary">Loyalty Program:</span>{" "}
            {loyaltyPoints > 0
