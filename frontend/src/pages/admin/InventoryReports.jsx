  ...
  const handleExport = () => {
    try {
      const doc = generateReportPdf(
        ...
        filtered.map((r) => [r.category, r.totalItems.toString(), r.value, r.turnover])
      );
        ...
        <Table>
          <TableHeader>
            <tr className="border-b border-surface-container dark:border-neutral-800">
              ...
              <TableHead className="px-4 text-right">Total Items</TableHead>
              <TableHead className="px-4 text-right">Value</TableHead>
              <TableHead className="px-4 text-right">Turnover Rate</TableHead>
            </tr>
          </TableHeader>
          <TableBody>
            {filtered.map((row, i) => (
              ...
              >
                <TableCell className="px-4 font-semibold">{row.category}</TableCell>
                <TableCell className="px-4 text-right">{row.totalItems.toLocaleString()}</TableCell>
                <TableCell className="px-4 text-right font-semibold">{row.value}</TableCell>
